...
const TaskCard = ({ task,updateTaskPoints,updateTaskTitle }: { task: Task, updateTaskPoints: (task:Task,points:number)=>void, updateTaskTitle: (task:Task,title:string)=>void }) => {
  ...
  return (
    <div
        ...
        className="flex flex-col bg-gray-200 justify-start gap-y-4 ml-4 items-start p-4 border border-black  rounded-lg mt-4  w-[450px]">
      <div className="text-xl text-black font-bold leading-[150%] ">
      {isEditing ?(
            <input autoFocus
                type="text"
                className="w-full"
